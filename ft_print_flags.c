        ...
        if (ft_strchr(format, '%'))
        {
			...
			else if (*format == 'u')
			{
				...
				if (precision == 1) // left_align == 1일 때 pad_right_blank 처리.
                {
                    if (print_len > precision_len)
                        ...
                        ft_pad_right_blank(width, precision_len, left_align);
                }
                else
                    ft_pad_right_blank(width, print_len, left_align);
			}
			else if (*format == 'x')
			{
                ...
                if (precision == 1)
                {
                    zero_padding = 0;
                    ft_pad_left_zero(width, print_len, left_align, zero_padding);
                    if (left_align != 1) //precision 있고 left_align이 아니면 오른쪽 정렬이라 left_black precision 고려해 실행한다.
                    {   
                        if (print_len > precision_len)
                            ft_pad_left_blank(width, print_len, left_align, zero_padding);
                        else
                    ...
                    //precision == 1이고 precision_len이 print_len보다 긴 경우 그 차이만큼 0으로 채워준다. (당연히 zero_padding은 0이다.)
                    ft_pad_left_zero(precision_len, print_len, 0, 1);
			...
			else if (*format == 'X')
			{
                ...
                if (precision == 1)
                {
                    //precision == 1이면 zero_padding = 0;이니까 zero_padding을 다 0으로 바꿔준다.
                    ft_pad_left_zero(width, print_len, left_align, 0);
                    if (left_align != 1) //precision 있고 left_align이 아니면 오른쪽 정렬이라 left_black precision 고려해 실행한다.
                    {   
                        if (print_len > precision_len)
                            ft_pad_left_blank(width, print_len, left_align, 0);
                        else
                            ft_pad_left_blank(width, precision_len, left_align, 0);
                    }
                    //precision == 1이고 precision_len이 print_len보다 긴 경우 그 차이만큼 0으로 채워준다. (당연히 zero_padding은 0이다.)
                    ft_pad_left_zero(precision_len, print_len, 0, 1);
                    ft_putnbr_base(ptr, "0123456789ABCDEF");
...
*/
    ...
    char str13[100] = "The value of (a, b) is (%30.19d, %07d)\n";
    int a = 200;
    ...
    char str14[100] = "The value of (h, i) is (%5.u, %020u)\n";
    int i = 200;
    long long j = -257;
    ...
    char str15[100] = "The value of (i, j) is (%-010.20x, %10x)\n";

    ft_printf(str15, i, j);
    printf(str15, i, j);

    char str16[100] = "The value of (a, b) is (%05.X, %-010X)\n";
    ft_printf(str16, i, j);
    printf(str16, i, j);
